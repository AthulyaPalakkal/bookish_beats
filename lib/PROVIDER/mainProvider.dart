...
void getCatgoryData(){
  db.collection("CATEGORY").get().then((value){
      ...
      categorynameslist.clear();
    {
      ...
      {
        categorynameslist.add(categoryadd(element.id,element.get("CATEGORY").toString()));
        notifyListeners();
...
void getBookDetails(){
  db.collection("BOOK_DETAILS").get().then((value){
      ...
      bookList.clear();
    {
      for(var element in value.docs)
      {
        bookList.add(productadd(element.id,
  ...
  void getEvents(){
    db.collection("EVENTS").get().then((value){
      ...
      {
        for(var element in value.docs) {
            eventsList.add(eventsadd(element.id,
  ...
  void getReview(){
    db.collection("REVIEWS").get().then((value){
        ...
        reviewlist.clear();
      {
        for(var element in value.docs)
        {
          reviewlist.add(reviewadd(element.id,
              element.get("IMAGE").toString(),
